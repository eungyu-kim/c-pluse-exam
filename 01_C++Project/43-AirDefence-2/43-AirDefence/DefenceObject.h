...
const float gravityAccelerate = 9.8;		// 중력 가속도

const float airplainSpeedPerSec = 30;			// 비행기 초속
const float airplainSize = 20;					// 비행기 크기
...
const int bombDropRate = 1;	// 비행기에서 폭탄 떨구는 확률
...
{		// velocity에 중력을 적용한 이후의 계산은 UniformMoving과 동일하므로
public:
	void Move(Vector &velocity, float interval, float &locX, float &height) override;
};
...
public:
	...
	// 지금 당장 리스트에 넣지 말고 reserve에 보관하고 있다가 나중에 처리
};